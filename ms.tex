...
We confirm the recently predicted ``pitchfork'' foreground signature in power spectra of high-redshift 21~cm measurements, wherein the interferometer is sensitive to large-scale structure on all baselines. This is due to the inherent response of a wide-field instrument and is characterized by enhanced power from foreground emission in Fourier modes adjacent to those considered to be most sensitive to the cosmological H~{\sc i} signal. In our recent paper, many signatures in the simulation which predicted this feature were validated against Murchison Widefield Array data but this key element was at or below the noise level. In this paper, we improve the data sensitivity through coherent averaging of 12 independent snapshots with identical instrument settings, and provide the first confirmation of the prediction with a signal-noise ratio $>10$. This wide-field effect can by mitigated by careful antenna design which suppresses sensitivity towards the horizon. With future instruments such as the Hydrogen Epoch of Reionization Array and the Square Kilometre Array, based on simple scaling relations, we estimate a suppression of $\gtrsim 40$~dB is required near the horizon for signal detection in cosmological H~{\sc i} modes in analyses that deploy the {\it foreground avoidance} technique.
...
For future work, we plan to extend our analysis to HERA. Based on our earlier study, such a dish will have a much desirable Fourier response from a foreground contamination viewpoint. One of our objectives is to forecast the per-baseline foreground contamination as a function of local sidereal time in order to tune the HERA observing strategy and data analysis to maximize sensitivity to the EoR signal.
