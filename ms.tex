...
We confirm our recent prediction of the ``pitchfork'' foreground signature in power spectra of high-redshift 21~cm measurements, wherein the interferometer is sensitive to large-scale structure on all baselines. This is due to the inherent response of a wide-field instrument and is characterized by enhanced power from foreground emission in Fourier modes adjacent to those considered to be most sensitive to the cosmological H~{\sc i} signal. In our recent paper, many signatures from the simulation which predicted this feature were validated against Murchison Widefield Array (MWA) data but this key element was at or below the noise level. In this paper, we improve the data sensitivity through coherent averaging of 12 independent snapshots with identical instrument settings, and provide the first confirmation of the prediction with a signal-noise ratio $>10$. This wide-field effect can by mitigated by careful antenna design which suppresses sensitivity towards the horizon. Simple models for antenna apertures proposed for future instruments such as the Hydrogen Epoch of Reionization Array and the Square Kilometre Array indicate they should suppress foreground contamination by $\sim 40$~dB relative to the MWA. However, detailed models are necessary to ensure the contamination stays below the expected cosmological signal.
